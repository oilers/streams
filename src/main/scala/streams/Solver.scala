...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if(initial.isEmpty) Stream.empty
      ...
      val more = for {
        ...
        next: (Block, List[Move]) <- newNeighborsOnly(neighborsWithHistory(block, history), explored)
      } yield next
  ...
  lazy val pathsFromStart: Stream[(Block, List[Move])] = {
    val startPath: (Block, List[Move]) = (startBlock, List())
    from(Stream(startPath), Set(startBlock))
  }
